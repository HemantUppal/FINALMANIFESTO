...
@app.route('/View')
def view():
    manifesto=Manifesto.query.filter_by(status='Ongoing' or 'In Progress')
    manifestc=Manifesto.query.filter_by(status='Completed')
    manifestn=Manifesto.query.filter_by(status='Not Completed')

    # Fetch total complaints count per scheme
    ...
    # Calculate top 5 complaints schemes
    top5 = promises_data[:5]
    top5_total = sum(item["complaints"] for item in top5) or 1  # avoid zero division
    for item in top5:
        item["percent"] = round((item["complaints"] / top5_total) * 100, 1)

    # Completed and not completed promises (replace logic as needed)
    completed_promises = [
        m.scheme_name for m in Manifesto.query.limit(5).all()
    ]
    not_completed_promises = [
        m.scheme_name for m in Manifesto.query.offset(5).limit(5).all()
    ]

    completed_percentage = round((len(completed_promises) / (len(completed_promises) + len(not_completed_promises))) * 100, 1)
    ...
    return render_template('view.html',manifesto=manifesto,
                           manifestc=manifestc,
                           manifestn=manifestn,
